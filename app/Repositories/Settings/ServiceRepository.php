...
class ServiceRepository
{
    ...
    function table($request)
    {
        ...
        return [
            'data' => $data->map(function ($data) {
                $action_button = '';
                if (hasPermission('service_edit')) {
                    $action_button .= '<a href="' . route('service.edit', $data->id) . '" class="dropdown-item"> ' . _trans('common.Edit') . '</a>';
                }
                if (hasPermission('service_delete')) {
                    $action_button .= actionButton(_trans('common.Delete'), '__deleteAlert(`' . route('service.delete', $data->id) . '`)', 'delete');
                }
    ...
    function store($request)
    {
        try {
            ...
            $service->save();
            return $this->responseWithSuccess(_trans('message.Service created successfully.'), $service);
        } catch (\Throwable $th) {
    ...
    function update($request, $id)
    {
        try {
            ...
            if (!$service) {
                return $this->responseWithError(_trans('message.Service not found'), 'id', 404);
            }
            $service->title                     = $request->title;
            $service->slug                     = Str::slug($request->title);
            $service->description              = $request->content;
            $service->status_id               = $request->status;
            $service->save();
            return $this->responseWithSuccess(_trans('message.Service Updated successfully.'), $service);
        } catch (\Throwable $th) {
            return $this->responseWithError($th->getMessage(), [], 400);
        }
    }
    ...
    public function statusUpdate($request)
    {
        try {
            ...
            if (@$request->action == 'active') {
                ...
                return $this->responseWithSuccess(_trans('message.Service activate successfully.'), $menu);
            }
            if (@$request->action == 'inactive') {
                $menu = $this->service->whereIn('id', $request->ids)->update(['status_id' => 4]);
                return $this->responseWithSuccess(_trans('message.Service inactivate successfully.'), $menu);
            }
            return $this->responseWithError(_trans('message.Service failed'), [], 400);
        } catch (\Throwable $th) {
            return $this->responseWithError($th->getMessage(), [], 400);
        }
    }
