...
use App\Mail\LeaveReferredMail;
...
class LeaveRequestRepository
{
    ...
    protected AssignLeaveRepository $assignLeaveRepository;
    public $leaveService;

    public function __construct(
        LeaveRequest $leaveRequest,
        UserRepository $userRepository,
        AssignLeaveRepository $assignLeaveRepository
    ) {
        $this->leaveRequest = $leaveRequest;
        $this->userRepository = $userRepository;
        $this->assignLeaveRepository = $assignLeaveRepository;
    }
    ...
    public function leaveDetailsInformation($request, $id)
    {
        ...
        if ($user) {
            ...
            if ($leaveRequest) {
                ...
                $data['image_url'] = $leaveRequest->image_url;
        ...
        try {
            ...
            if ($userDepartment) {
                ...
                if (settings('leave_assign') == 1) {
                    ...
                    $days_diff = $this->dateDiff($request->leave_from, $request->leave_to);
                    if($rest_days < $days_diff){
                        return $this->responseWithError('You have exceeded your allowed leave days.', [], 400);
                    }
                }
                ...
                if ($leaveType) {
                    ...
                    if ($user_check || $department_check) {
                        ...
                        if ($user) {
                            ...
                            $leaveRequest->image_url = $request->image_url;
                            $leaveRequest->save();
                            ...
                            if ($leaveRequest->user->manager_id != null) {
                                ...
                                foreach($leaveRequest->user->hrUserIds($leaveRequest->user) ?? [] as $hr_id) {
                                    $hrUser = User::where('id', $hr_id)->first();
                
                                    $this->sendChannelFirebaseNotification('user' . $hr_id, 'leave_requested', null, route('leaveRequest.index'), $details['title'], $details['body'], null);
                                    sendDatabaseNotification($hrUser, $details);
                                    try {
                                        Mail::to($hrUser->email)->send(new LeaveRequestMail($hrUser, $details));
                                    } catch (\Throwable $th) {
                                        Log::error($th);
                                    }
                                }
    ...
    public function approveOrRejectOrCancel($id, $status): bool
    {
        ...
        if ($leaveRequest) {
            ...
            if ($status == 1) {
                ...
                $notify_body = $leaveRequest->assignLeave->type->name . " Approved by: " . auth()->user()->name;
                $details = [
                    'title' => _trans('response.Leave Request Approved'),
                    'body' => $notify_body,
                    'actionText' => 'View',
                    'actionURL' => [
                        'app' => 'leave_request',
                ...
                sendDatabaseNotification($leaveRequest->user, $details);
                // leave year table update
                $this->assignLeaveRepository->updateAnnualLeaveCount($leaveRequest->assignLeave, $leaveRequest->days);
                ...
                $notify_body = $leaveRequest->assignLeave->type->name . " Rejected by: " . auth()->user()->name;
                $details = [
                    'title' => _trans('response.Leave Request Rejected'),
                    'body' => $notify_body,
                    'actionText' => 'View',
                    'actionURL' => [
                        'app' => 'leave_request',
                ...
                $notify_body = $leaveRequest->assignLeave->type->name . " Cancelled by: " . auth()->user()->name;
                $details = [
                    'title' => _trans('response.Leave Request Cancelled'),
                    'body' => $notify_body,
                    'actionText' => 'View',
                    'actionURL' => [
                        'app' => 'leave_request',
                ...
                $notify_body = $leaveRequest->assignLeave->type->name . " Referred by: " . auth()->user()->name;
                $details = [
                    'title' => _trans('response.Leave Request Referred'),
                    'body' => $notify_body,
                    'actionText' => 'View',
                    'actionURL' => [
                        'app' => 'leave_request',
                ...
                foreach($leaveRequest->user->hrUserIds($leaveRequest->user) ?? [] as $hr_id) {
                    ...
                    $this->sendChannelFirebaseNotification('user' . $hr_id, 'leave_referred', null, route('leaveRequest.index'), $details['title'], $details['body'], null);
                    sendDatabaseNotification($hrUser, $details);
                    try {
                        Mail::to($hrUser->email)->send(new LeaveReferredMail($hrUser, $details));
                    } catch (\Throwable $th) {
                        Log::error($th);
                    }
                }
