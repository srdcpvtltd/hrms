...
class MeetingRepository
{
    ...
    public function meetingList(): \Illuminate\Http\JsonResponse
    {
        $meeting = $this->model->query()
            ...
            ->where(function ($query) {
                $query->where(['meetings.company_id' => $this->companyInformation()->id, 'meetings.user_id' => auth()->id()])
                      ->orWhere(['meeting_participants.participant_id' => auth()->id()]);
            })
            // ->where(['company_id' => $this->companyInformation()->id, 'user_id' => auth()->id()])
            ->when(request()->has('month'), function ($query) {
                $query->where('meetings.date', 'LIKE', '%' . request('month') . '%');
            })
            ->select('meetings.*','meeting_participants.*','meetings.id as id')
    ...
    public function show($id): \Illuminate\Http\JsonResponse
    {
        ...
        $data['image_url'] = @$meeting->image_url ?? "";
        return $this->responseWithSuccess('Meeting list', $data, 200);
    }
    ...
    public function store($request): \Illuminate\Http\JsonResponse
    {
        ...
        $meeting->image_url = $request->image_url;
