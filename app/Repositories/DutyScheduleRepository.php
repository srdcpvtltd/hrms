...
class DutyScheduleRepository
{
    ...
    {
        return Shift::query()->where('company_id', getCurrentCompany())->get();
    }

    public function index()
    {
        $data['title'] = 'Duty Schedule';
        $data['schedules'] = DutySchedule::query()->where('company_id', getCurrentCompany())->get();
    ...
    public function create()
    {
        ...
        $data['shifts'] = Shift::query()->where('company_id', getCurrentCompany())->where('status_id', 1)->get();
        return $data;
    }
        ...
        try {
            foreach ($request->shift_id as $key => $item) {
                if ($this->isExistsWhenStore($this->dutySchedule, 'shift_id', $item)) {
                    $request['company_id'] = getCurrentCompany();
    ...
    {
        $dutySchedule = DutySchedule::query()->where('company_id', getCurrentCompany());
    ...
    function table($request)
    {
        ...
        // Log::info($request);
        $data = $this->dutySchedule->query()->where('company_id', getCurrentCompany());
