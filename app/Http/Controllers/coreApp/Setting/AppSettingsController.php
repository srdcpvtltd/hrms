...
class AppSettingsController extends Controller
{
    ...
    public function getDocumentRequestList(Request $request){
        try {
            return $this->appSettings->getDocumentRequest();
        } catch (\Throwable $th) {
            return response()->json(['message' => _trans('response.Something went wrong.'), 'status' => 500]);
        }
    }
    ...
    public function submitDocumentRequest(Request $request){
        try {
            $this->appSettings->submitDocumentRequest($request);
            return response()->json(['message' => 'New Document Request Created Successfully', 'status' => 200]);
        } catch (\Throwable $th) {
            return response()->json(['message' => _trans('response.Something went wrong.'), 'status' => 500]);
        }
    }
