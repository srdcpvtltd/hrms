...
class TaskFileService extends BaseService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $task_files->company_id = getCurrentCompany();
            $task_files->branch_id = getCurrentBranch();
            ...
            \App\Models\TaskManagement\TaskActivity::CreateActivityLog(getCurrentCompany(), $request->task_id, auth()->id(), 'Created File', getCurrentBranch())->save();
            DB::commit();
    ...
    public function commentStore($request)
    {
        ...
        try {
            ...
            $comment->company_id = getCurrentCompany();
            $comment->branch_id = getCurrentBranch();
            ...
            \App\Models\TaskManagement\TaskActivity::CreateActivityLog(getCurrentCompany(), intval($task_file->task_id), auth()->id(), 'Created File Comments', getCurrentBranch())->save();
            DB::commit();
    ...
    function delete($id)
    {
        $file = $this->model->where(['id' => $id, 'company_id' => getCurrentCompany(), 'branch_id' => getCurrentBranch()])->first();
        if (!$file) {
        ...
        try {
            ...
            $file->delete();            
            \App\Models\TaskManagement\TaskActivity::CreateActivityLog(getCurrentCompany(), $file->task_id, auth()->id(), 'Deleted File', getCurrentBranch())->save();
    ...
    public function destroyAll($request)
    {
        try {
            if (@$request->ids) {
                $category = $this->model->where('company_id', getCurrentCompany())->where('branch_id', getCurrentBranch())->whereIn('id', $request->ids)->get();
