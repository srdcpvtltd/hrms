            ...
            <div class="modal-body">
                <div class="row p-5">
                    <div class="col-md-12">
                        ...
                        <p>
                            <strong>{{ _trans('payment_method.Deal Amount') }}</strong> : {{  $data['payment']->user->agreement->amount }} {{ _trans('common.Tk') }}
                        </p>
                        ...
                        <form action="{{ $data['url'] }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="amount" class="form-control"  step=0.01 required value="{{ $data['payment']->amount }}">
                            </div>
                            <div class="form-group">
                                {{Form::label('payment_note','Note', ['class' => 'form-label required']) }}
                                <input type="text" name="payment_note" class="form-control" required placeholder="{{ _trans('payment.Enter payment note') }}">
                            </div>
                            <div class="form-group text-right">
                                <button type="submit" class="btn btn-primary pull-right"><b>{{
                                         _trans('common.Update') }}</b></button>
                            </div>
                        </form>
