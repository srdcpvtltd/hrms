@if (hasPermission('award_menu') && hasFeature('awards'))
    <li class="sidebar-menu-item {{ set_menu([route('award.index')]) }}">
        <a href="javascript:void(0)" class="parent-item-content has-arrow {{ menu_active_by_route(['award_type.index','award.index','award.create','award.edit','award.view']) }}">
        ...
        <ul class="child-menu-list {{ set_active(['admin/award*']) }}">
            ...
            @if (hasPermission('award_list'))
                <li class="sidebar-menu-item {{ menu_active_by_route(['award.index','award.create','award.edit','award.view']) }}">
