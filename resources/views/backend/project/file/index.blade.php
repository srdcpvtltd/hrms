                                ...
                                <div class="align-self-center">
                                    @if (hasPermission('project_files_create'))
                                        <button href="javascript:;"
                                            onclick="viewModal(`{{ route('project.file.create', 'project_id=' . $data['view']->id) }}`)"
                            ...
                            <div class="align-self-center">
                                <div class="search-box d-flex">
                                    <input class="form-control" placeholder="{{ _trans('common.Search') }}"
                                        name="search" onkeyup="fileTaskTable()" autocomplete="off" />
                        ...
                        <div class="col-lg-3 col-sm-12 col-12">
                            <div class="single-post card ot-card">
                                ...
                                <div class="news-cnt mt-20">
                                    <a target="_blank" href="{{ route('project.file.download', 'project_id=' . $data['view']->id) . '&file_id=' . $data['file']->id }}"
                                        class="link-black text-sm text-primary"><i class="fa fa-download"></i>
                                        {{ @$data['file']->subject }} </a>
                                    <br>
                                    <span>{{ _trans('project.Posted on') }}
                                        {{ \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $data['file']->created_at) }}</span>
                                    <br>
                    ...
                    <div id="discussion-comments" class="tc-content jquery-comments">
                        <div class="card-body">
                                        ...
                                        @if (is_null($comment->comment_id))
                                            ...
                                            @if (@$comment->childComments)
                                                @foreach ($comment->childComments as $childComment)
                                                    <div
                                                        class="post ml-5 dis-{{ $comment->id }} c-none clearfix ml-70">
                                                        <div class="user-block d-flex align-items-center gap-2">
                                                            <img class="img-circle img-bordered-sm"
                                                ...
                                                @if (hasPermission('project_file_comment'))
                                                    <div class="dis-{{ $comment->id }} c-none ml-70 mb-20">
                                                        <div class="form-group">
                                                            <textarea name="comment-{{ $comment->id }}" id="comment-{{ $comment->id }}"
                                                                class="form-control ck_editor ot-input" row="5" col="5"> </textarea>

                                                        </div>
                                                        <div class="error-message-{{ $comment->id }}">
                                                        </div>
                                                        <div
                                                            class="form-group float-right d-flex justify-content-end my-4">
                                                            <button class="btn btn-gradian"
                            ...
                            @if (hasPermission('project_file_comment'))
                                ...
                                <div class="form-group">
                                    <textarea name="comment-" id="comment-" class="form-control ck_editor ot-input " row="5" col="5"> </textarea>
                                </div>
                                <div class="error-message-"></div>
                                <div class="form-group float-right d-flex justify-content-end mt-24">
                                    <button class="btn btn-gradian"
